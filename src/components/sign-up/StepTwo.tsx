...
export const StepTwo = ({
  ...
  const handleNextStep = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (!validateNonEmptyFields(stepFields)) {
      ...
      if (body.country === "Uruguay") {
        setError(
          "Lamentablemente, aún no ofrecemos nuestros servicios en Uruguay."
        );
        setFieldsWithErrors([...fieldsWithErrors, "country"]);
        return;
      }
      setIsLoading(true);
      setTimeout(() => {
  ...
  return (
    <div className="flex flex-col items-center w-full md:min-h-[450px] ">
      ...
      {isLoading ? (
        <div className="flex flex-col w-full space-y-2 my-16 py-10">
          <Loader />
        </div>
      ) : (
        <>
